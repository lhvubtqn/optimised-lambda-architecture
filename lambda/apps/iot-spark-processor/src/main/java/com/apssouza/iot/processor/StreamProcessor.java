...
public class StreamProcessor implements Serializable {
    ...
    public StreamProcessor filterVehicle() {
        ...
        JavaPairDStream<String, IoTData> map = mapToPair(transformedStream);
        JavaPairDStream<String, IoTData> key = reduceByKey(map);
        JavaMapWithStateDStream<String, IoTData, Boolean, Tuple2<IoTData, Boolean>> state = mapWithState(key);
    ...
    private JavaDStream<Tuple2<IoTData, Boolean>> filterByState(final JavaMapWithStateDStream<String, IoTData, Boolean, Tuple2<IoTData, Boolean>> state) {
        JavaDStream<Tuple2<IoTData, Boolean>> dsStream = state .filter(tuple -> tuple._2.equals(Boolean.FALSE));
    ...
    private JavaMapWithStateDStream<String, IoTData, Boolean, Tuple2<IoTData, Boolean>> mapWithState(final JavaPairDStream<String, IoTData> key) {
        ...
        JavaMapWithStateDStream<String, IoTData, Boolean, Tuple2<IoTData, Boolean>> dStream = key.mapWithState(stateFunc);
        dStream.print();
        return dStream;
    }
    ...
    private JavaPairDStream<String, IoTData> mapToPair(final JavaDStream<IoTData> stream){
        JavaPairDStream<String, IoTData> dStream = stream.mapToPair(iot -> new Tuple2<>(iot.getVehicleId(), iot));
        dStream.print();
        return dStream;
    }
